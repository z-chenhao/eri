...
A deterministic pre-gate checks schema, credentials, authority, approval, cost-bearing side effects, and Receipt invariants. Then a general LLM Judge uses its own strict System prompt to reconstruct the whole conversation arc and evaluate semantic completion, factual support, Tool/Receipt consistency, Skill method, risk, clarity, interpersonal meaning, Soul/delivery style, and whether the message honestly distinguishes draft from action. The exact generation transcript ends with the assistant Candidate; bounded evaluation metadata belongs to the Judge System context and Runtime never appends a synthetic Eval user Message. Runtime does not encode task-domain or emotional `case-when` replies. Judge requests disable provider reasoning and first request native structured output. If that mode returns empty final content, the next bounded attempt keeps the exact transcript and stable System prefix but omits the native JSON response mode; the strict JSON decoder, enum validation, Memory allowlist, and fail-closed exhaustion remain unchanged. Private provider reasoning is never parsed as a decision or copied into a retry.
