...
A deterministic pre-gate checks schema, credentials, authority, approval, cost-bearing side effects, and Receipt invariants. Then a general LLM Judge uses its own strict System prompt to reconstruct the whole conversation arc and evaluate semantic completion, factual support, Tool/Receipt consistency, Skill method, risk, clarity, interpersonal meaning, Soul/delivery style, and whether the message honestly distinguishes draft from action. The authoritative generation transcript still ends with the assistant Candidate and Runtime never appends a synthetic Eval turn to Conversation or the Agent checkpoint. For the separate Judge call, Runtime projects that transcript, bounded Candidate context, and evaluation metadata into one JSON evidence envelope sent as the Judge's sole user input. This prevents the provider from treating Eval as continuation of the user's chat. Runtime does not encode task-domain or emotional `case-when` replies. Judge keeps provider reasoning enabled and retains native structured output on every bounded attempt. If DeepSeek returns empty, whitespace-only, or invalid content, the next attempt keeps the same envelope and appends a strict protocol-repair instruction to the Judge System prompt, as the provider recommends modifying the prompt rather than dropping JSON Output. The strict JSON decoder, enum validation, Memory allowlist, and fail-closed exhaustion remain unchanged. All generation `reasoning_content` is omitted from the Judge envelope; Judge response reasoning is never parsed as a decision or copied into a retry.
