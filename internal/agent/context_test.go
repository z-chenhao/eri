...
func TestBuildMessagesPreservesRawConversationWithoutTaskWrappers(t *testing.T) {
	...
	if len(messages) != 2 || messages[0].Content != "Finish the approved draft." || messages[1].Content != "Start a separate research task." {
...
func TestMemoryIsInsertedImmediatelyBeforeTriggerInteraction(t *testing.T) {
	records := []ContextRecord{{ID: "user-1"}, {ID: "assistant-1"}, {ID: "trigger-1"}}
	messages := []Message{{Role: "user", Content: "Earlier"}, {Role: "assistant", Content: "Reply"}, {Role: "system", Content: "Reminder due"}}
	memoryMessage := Message{Role: "system", Content: "<relevant_memory_context>remember this</relevant_memory_context>"}

	got := insertBeforeSourceInteraction(messages, records, "trigger-1", 0, memoryMessage)
	if len(got) != 4 || got[2].Role != memoryMessage.Role || got[2].Content != memoryMessage.Content || got[3].Content != "Reminder due" {
		t.Fatalf("messages=%+v", got)
	}
}

func TestMemoryInsertionAccountsForPersistentCheckpoint(t *testing.T) {
	records := []ContextRecord{{ID: "old-1"}, {ID: "old-2"}, {ID: "kept-1"}, {ID: "trigger-1"}}
	messages := []Message{{Role: "system", Content: "checkpoint"}, {Role: "assistant", Content: "kept"}, {Role: "user", Content: "trigger"}}
	memoryMessage := Message{Role: "system", Content: "memory"}

	got := insertBeforeSourceInteraction(messages, records, "trigger-1", 2, memoryMessage)
...
func TestLoopCompactionDoesNotReintroduceRemovedPromptWrappers(t *testing.T) {
	...
	messages = append(messages,
		...
		Message{Role: "system", Content: "A durable commitment is due. Check the monitored sources."},
	)
